# Zanatta Gregory — Gameplay Programmer & Tools Developer

A 3rd-year game development student at Objectif3D specializing in gameplay programming, multiplayer systems, and production tool development. I work mainly with C++, C#, Unreal Engine, and Unity.

## About me
- Currently on a long-term academic game project made with advanced Unreal Engine.
- Interested in collaborating on gameplay systems, networking, and production tooling projects.

## 🛠 Skills
| Tech | Usage |
|------|-------------|
| ![C++](https://img.shields.io/badge/C++-00599C?logo=cplusplus&logoColor=white) | Core gameplay programming, engine systems, Unreal Engine development |
| ![SFML](https://img.shields.io/badge/SFML-8CC445?logo=sfml&logoColor=white) | Custom 2D game development and rendering systems |
| ![Unreal Engine](https://img.shields.io/badge/Unreal%20Engine-313131?logo=unrealengine&logoColor=white) | 2D/3D game production using Blueprint and C++ |
| ![C#](https://img.shields.io/badge/C%23-239120?logo=csharp&logoColor=white) | Game logic, tooling, and reusable libraries |
| ![WPF](https://img.shields.io/badge/WPF-512BD4?logo=.net&logoColor=white) | Desktop tool and editor application development |
| ![XAML](https://img.shields.io/badge/XAML-0C54C2?logo=windows&logoColor=white) | UI layout and interface styling for WPF tools |
| ![Unity](https://img.shields.io/badge/Unity-000000?logo=unity&logoColor=white) | 2D/3D game development with C# scripting |
| ![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white) | Pipeline tools, Unreal/Maya scripting, 2D prototyping |
| Multiplayer / Networking | Gameplay replication, client/server logic, online systems in Unreal, Unity, and SFML |

## 🎮 Technical focus
- Gameplay systems architecture
- 3C design (Character / Camera / Controls)
- Multiplayer & networked gameplay

## 🔗 Links
- Portfolio:
- LinkedIn: 

## 📫 Contact
Preferred: EMAIL: gregza@hotmail.fr  
Open to collaboration
